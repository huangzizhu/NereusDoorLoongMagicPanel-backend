...
- [特权模型 V2 部署与集成](docs/privilege-model-v2-deploy.md)
- [提示词注入防护设计](docs/prompt-injection-defense.md)
- [系统提示词构成与修改指南](docs/system-prompt.md)

## 本轮更新摘要

- **无人值守 Agent 执行**：定时任务和自动巡检支持独立后台会话、执行历史、巡检报告与 source 来源标记；调度会话在重连后仍可追溯。
- **授权闭环**：新增 allowedCommands 命令前缀白名单和 shell 控制字符拦截。预授权未覆盖时生成 authorization.requested 事件，任务跳过当前步骤继续执行，管理员可通过 sudo nereus approve CODE 授权；批准后签发受限 JIT token，并按任务或巡检策略跨运行生效。
- **LLM 稳定性**：结构化输出统一做 JSON/schema 校验，失败时按配置重试；工具调用、后台任务和辅助 LLM 请求支持超时与重试参数，提示词资源从 conf/prompts/ 外置加载。
- **分层注入防护**：在原有规则拦截和执行层审批之外，加入部署级金丝雀、LLM 分类器抽检/全量检测、工具输出过滤以及安全告警与审计事件。
- **运维稳定性**：优化 Docker 统计的批量查询、文件属主解析、告警排序分页，并为防火墙和特权代理补充超时与错误边界。
- **组织运维经验库**：新增经验包的管理 API、附件导入导出、知识摘要、Agent 检索/详情/沉淀工具以及对应的知识提示词；经验包功能本身纳入项目实现，运行时导出的 zip 和演示数据不纳入提交。

本轮整理不包含运行时导出的经验包 zip、经验包演示场景、临时演示目录、运行日志和测试素材。
